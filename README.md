...
FinPay microservice — notification-service. Part of the [finpay-lab](https://github.com/finpay-lab) multi-repository
...
- Async email/SMS/push dispatch, failure-isolated, outbox consumer + channel dispatch (FP-19).
- **AI-2 (FP-59):** `MessageGenerator` — LLM-generated personalized copy with static-template fallback.
...
Images are built, `kind load`ed as `finpaylab/notification-service:fp9`, and rolled out via the
`finpay-services` Argo CD ApplicationSet. The API entrypoint is the **gateway**
(port 8080); this service is reached internally on port 8091.
